...
class Token_stream {
public:
    Token_stream(std::istream& istr);   // make a Token_stream that reads from std::cin
...
Token_stream::Token_stream(std::istream& istr) : full(false), buffer(0), istr{istr} {}    // no Token in buffer
...
Token Token_stream::get() { // read characters from std::cin and compose a Token
    ...
    char ch;
    istr >> ch;          // note that >> skips whitespace (space, newline, tab, etc.)
    ...
    switch (ch) {
        ...
        {
            istr.putback(ch);// put digit back into the input stream
            double val;
            istr >> val;     // read a floating-point number
...
Token_stream ts(std::cin);        // provides get() and putback()
